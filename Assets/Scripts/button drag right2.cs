...
public class buttondragright2 : MonoBehaviour
{
    ...
    void Update(){
        ...
        if (Input.GetMouseButtonUp(0) && isDraggingRight)
        {
            ...
            if (buttonOriginalPos() && transform.position != startingPointRight.position){
        ...
        if (gravityswitch.switched && !switched1){
            buttonRightPos = 2;
            switched1 = true;
        } 
    }

    private bool buttonOriginalPos(){
        if (gravityswitch.switched){
            return transform.position.y > 2.55f;
        } else {
            return transform.position.y < -2.55f;
        }
    }
