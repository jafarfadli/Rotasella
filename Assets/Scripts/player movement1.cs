...
public class PlayerMovement1 : MonoBehaviour{
    ...
    private void Update(){
        ...
        if (leftInput() > 0 && leftButton.position == leftStartingPoint.position){
            int moveIndex = buttonLeft.GetComponent<buttondragleft1>().buttonLeftPos;
            if (moveIndex == 0){
                ...
                // body.linearVelocity = new Vector2(body.linearVelocity.x, horizontalInput * jumpPower * (-1));
            }
        }

        if (rightInput() > 0 && rightButton.position == rightStartingPoint.position){
            int moveIndex = buttonRight.GetComponent<buttondragright1>().buttonRightPos;
            if (moveIndex == 0){
                body.linearVelocity = new Vector2(rightInput() * speed,body.linearVelocity.y);
                transform.localScale = new Vector3(1,1,1);
                anim.SetBool("move", true);
            } else if (moveIndex == 1){
                // body.linearVelocity = new Vector2(body.linearVelocity.x, horizontalInput * jumpPower);
            } else if (moveIndex == 2){
